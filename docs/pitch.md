...
It watches namespace-scoped pod signals and can optionally add read-only node state, diagnoses common failures like CrashLoopBackOff, OOMKilled, Pending pods, and NodeNotReady, and then routes each incident through a LangGraph workflow: observe, detect, diagnose, plan, safety gate, execute, and explain.
